...
console.log('(custom.js, raw data) check in user list:', check_in_users_history);

var rawData = check_in_users_history;
...
rawData.forEach(entry => {
  ...
  labels.push(checkInTime.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' }));

  // Calculate total hours since midnight and round to two decimal places
    var totalHours = (checkInTime.getHours() + checkInTime.getMinutes() / 60).toFixed(2);
    data.push(parseFloat(totalHours));

  var date_day = checkInTime.getDate();
  var month = checkInTime.getMonth(); // Month (0-11)
  var year = checkInTime.getFullYear(); // Four-digit year
  var full_calendar = `${month + 1}-${date_day}-${year}`; // Adding 1 to month to display it as 1-12 range
  full_date.push(full_calendar);
});

console.log("Processed Labels (Time):", labels);
console.log("Processed Data (Total Minutes):", data);
console.log("Processed full (Date):", full_date);
